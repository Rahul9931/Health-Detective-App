...
class Dieases_Detection_Activity : AppCompatActivity() {
    ...
    var symListClear:Boolean = false
    /*val symList = mutableListOf(
        "itching",
        "skin_rash",
    ...
    val symListInFloat = mutableListOf(
        0,
        0,
        0,
        0,
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        // Fetching All Symptoms
        val sym = "symptoms_HinEng"
        val inputSym = application.assets.open(sym).bufferedReader().use { it.readText() }
        symList = inputSym.split("\n")
        Log.d("check_symListtop","${symList}")
        ...
        binding.btnSymptoms.setOnClickListener {
            ...
            Log.d("check_usersymmList","${usersymList}")
        }
            ...
            if (usersymList.isNotEmpty()) {
                ...
                for (k in 0..symList.size - 1) {
                    for (z in usersymList) {
                        if (symList[k].trim().equals(z.trim())) {
                            Log.d("check_symList","${symList[k]}")
                            Log.d("check_usersymList","${z}")
                            symListInFloat[k] = 1
                        }
                    }
                }
                symListClear = true
                Log.d("check_symListInFloat","${symListInFloat}")
    ...
    private fun setAdapterForDieasesAccuracy(
        sortedKey: List<Float>,
        sortedValue: MutableList<String>
    ) {
